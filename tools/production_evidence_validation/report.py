...
def _case_memory_coverage(runtime_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "missingRows": coverage.get("missingRows") or [row for row in rows if row.get("status") == "MISSING"],
        "nextCollectionQueue": coverage.get("nextCollectionQueue") or [],
        ...
        "targetSampleCount": coverage.get("targetSampleCount") or 0,
        "observedSampleCount": coverage.get("observedSampleCount") or sum(int(value or 0) for value in (coverage.get("categoryCounts") or {}).values()),
        "remainingTargetSampleCount": coverage.get("remainingTargetSampleCount") or 0,
