...
input bool KeepRunningForRuntimeProbe = false;
input int RuntimeProbeIntervalSeconds = 15;
input int RuntimeProbeWarmupSeconds = 45;
...
const string RUNTIME_PROBE_FILE_NAME = "QuantGod_HFMCryptoRuntimeProbe.json";
...
string BuildStandaloneHfmCryptoSymbolSpecsJson()
{
   ...
   json += "  \"source\": \"MQL5_SYMBOLINFO_READONLY_STANDALONE_EA\",\n";
   json += "  \"expert\": \"QuantGod_HFMCryptoSpecExporterEA.mq5\",\n";
   json += "  \"enabled\": true,\n";
...
void ExportStandaloneHfmCryptoRates()
{
   ...
   payload += "  \"source\": \"MQL5_COPYRATES_READONLY_STANDALONE_EA\",\n";
   payload += "  \"expert\": \"QuantGod_HFMCryptoSpecExporterEA.mq5\",\n";
   ...
   if(handle == INVALID_HANDLE)
   {
      Print("QuantGod standalone HFM crypto rates EA export failed: ", manifestPath, " error=", GetLastError());
      return;
   }
   FileWriteString(handle, payload);
   FileClose(handle);
   Print("QuantGod standalone HFM crypto rates EA exported: ", manifestPath, " rows=", totalRows);
}

void ExportStandaloneHfmCryptoSpecs()
{
   FolderCreate(ExportFolderName);
   ...
   if(handle == INVALID_HANDLE)
   {
      Print("QuantGod standalone HFM crypto specs EA export failed: ", relativePath, " error=", GetLastError());
      return;
   }
   FileWriteString(handle, payload);
   FileClose(handle);
   Print("QuantGod standalone HFM crypto specs EA exported: ", relativePath);
   ExportStandaloneHfmCryptoRates();
}

void AppendRuntimeProbeSymbolJson(string symbol, string &json, int &count)
{
   if(StringLen(symbol) == 0)
      return;
   ...
   json += "      \"canonicalSymbol\": " + JsonString(NormalizeHfmCryptoCanonical(symbol)) + ",\n";
   json += "      \"source\": \"MQL5_SYMBOLINFO_READONLY_STANDALONE_EA_RUNTIME_PROBE\",\n";
   json += "      \"description\": " + JsonString(description) + ",\n";
   json += "      \"path\": " + JsonString(path) + ",\n";
   json += "      \"visible\": " + JsonBool(visible != 0) + ",\n";
   json += "      \"selected\": " + JsonBool(selected != 0) + ",\n";
   ...
   json += "      \"spread\": " + IntegerToString((int)spread) + ",\n";
   json += "      \"spreadFloat\": " + JsonBool(spreadFloat != 0) + ",\n";
   json += "      \"spreadPoints\": " + JsonDouble(spreadPoints) + ",\n";
   json += "      \"tradeMode\": " + IntegerToString((int)tradeMode) + ",\n";
   json += "      \"tradeEnabled\": " + JsonBool(tradeMode > 0) + ",\n";
   json += "      \"tickOk\": " + JsonBool(tickOk && tick.ask > tick.bid) + ",\n";
   json += "      \"bid\": " + JsonDouble(tickOk ? tick.bid : 0.0, digits > 0 ? (int)digits : 5) + ",\n";
   json += "      \"ask\": " + JsonDouble(tickOk ? tick.ask : 0.0, digits > 0 ? (int)digits : 5) + ",\n";
   json += "      \"time\": " + IntegerToString((long)(tickOk ? tick.time : 0)) + "\n";
   json += "    }";
   count++;
}

string BuildStandaloneHfmCryptoRuntimeProbeJson()
{
   string candidates[] = {
      _Symbol,
      "#BTCUSD", "#ETHUSD", "#SOLUSD", "#XRPUSD", "#DOGEUSD", "#LTCUSD",
   ...
   for(int i = 0; i < ArraySize(candidates) && count < MaxSymbolsToExport; i++)
   {
      ...
      visited += symbol + "|";
      scanned++;
      if(symbol == _Symbol || LooksLikeHfmCryptoSymbol(symbol))
         AppendRuntimeProbeSymbolJson(symbol, rows, count);
   }
   ...
   json += "  \"source\": \"MQL5_SYMBOLINFO_READONLY_STANDALONE_EA_RUNTIME_PROBE\",\n";
   json += "  \"expert\": \"QuantGod_HFMCryptoSpecExporterEA.mq5\",\n";
   json += "  \"generatedAtServer\": " + JsonString(TimeToString(now, TIME_DATE | TIME_SECONDS)) + ",\n";
   json += "  \"chartSymbol\": " + JsonString(_Symbol) + ",\n";
   json += "  \"candidateSymbolsScanned\": " + IntegerToString(scanned) + ",\n";
   json += "  \"symbolCount\": " + IntegerToString(count) + ",\n";
   json += "  \"symbols\": [\n";
   json += rows;
   ...
   json += "    \"livePilotMode\": false,\n";
   json += "    \"tradeAllowed\": false,\n";
   json += "    \"tradeStatus\": \"HFM_CRYPTO_RUNTIME_PROBE_ONLY\"\n";
   json += "  },\n";
   json += "  \"safety\": {\n";
   json += "    \"readOnly\": true,\n";
   json += "    \"orderSendAllowed\": false,\n";
...
void ExportStandaloneHfmCryptoRuntimeProbe()
{
   FolderCreate(ExportFolderName);
   string relativePath = ExportFolderName + "\\" + RUNTIME_PROBE_FILE_NAME;
   string payload = BuildStandaloneHfmCryptoRuntimeProbeJson();
   ResetLastError();
   int handle = FileOpen(relativePath, FILE_WRITE | FILE_TXT | FILE_ANSI);
   if(handle == INVALID_HANDLE)
   {
      Print("QuantGod standalone HFM crypto runtime probe export failed: ", relativePath, " error=", GetLastError());
      return;
   }
   FileWriteString(handle, payload);
   FileFlush(handle);
   FileClose(handle);
   Print("QuantGod standalone HFM crypto runtime probe exported: ", relativePath);
}

bool HfmRuntimeProbeHasLiveTick()
{
   MqlTick tick;
   ZeroMemory(tick);
...
void WarmupStandaloneRuntimeProbe()
{
   int warmupSeconds = RuntimeProbeWarmupSeconds;
   if(warmupSeconds <= 0)
      return;
   if(warmupSeconds > 120)
      warmupSeconds = 120;

   int waited = 0;
   while(waited < warmupSeconds && !IsStopped())
   {
      if(HfmRuntimeProbeHasLiveTick())
         return;
      Sleep(1000);
      waited++;
      ExportStandaloneHfmCryptoRuntimeProbe();
   }
}
// Standalone HFM Crypto Spec Exporter EA END

int OnInit()
{
   ExportStandaloneHfmCryptoRuntimeProbe();
   WarmupStandaloneRuntimeProbe();
   ExportStandaloneHfmCryptoSpecs();
   ExportStandaloneHfmCryptoRuntimeProbe();
   if(KeepRunningForRuntimeProbe)
   {
      int intervalSeconds = RuntimeProbeIntervalSeconds;
