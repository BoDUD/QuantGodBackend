...
class USDJPYAutonomousAgentTests(unittest.TestCase):
    def test_builds_stage_gated_agent_files_without_execution_rights(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(decision["safety"]["autoApplyAllowed"], "shadow_only")
            self.assertTrue(decision["safety"]["operatorApprovalRequired"])
            self.assertFalse(decision["safety"]["unattendedLiveExpansionAllowed"])
            self.assertFalse(decision["safety"]["liveExpansionAllowed"])
            ...
            self.assertTrue(patch["operatorApprovalRequired"])
            self.assertFalse(patch["unattendedLiveExpansionAllowed"])
            self.assertFalse(patch["liveExpansionAllowed"])
            ...
            self.assertIn("operatorApprovalRequired=true", ea_text)
            self.assertIn("unattendedLiveExpansionAllowed=false", ea_text)
            self.assertIn("liveExpansionAllowed=false", ea_text)
